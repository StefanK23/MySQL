...
#Exercise --12 SoftUni database-- 

SELECT `department_id`, min(`salary`) as 'minimun_salary' from employees WHERE department_id IN (2 , 5 , 7) 
AND hire_date > '2000/01/01' 
GROUP BY department_id  ORDER BY department_id; 

#Exercise --13-- 
CREATE TABLE `hight_paid_employee` 
AS 
SELECT * from employees WHERE salary > 30000 AND `manager_id` != 42; 

UPDATE `hight_paid_employee` 
SET `salary` = `salary` + 5000 Where department_id = 1;

SELECT `department_id` , AVG(`salary`) as 'avg_salary' FROM `hight_paid_employee` GROUP BY department_id ORDER BY department_id;
