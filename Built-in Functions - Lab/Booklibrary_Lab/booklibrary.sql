...
INSERT INTO books(author_id,title, year_of_release,cost) VALUES
    ...
    SELECT right(title, 10) from books; 
    SELECT title from books;
   
    SELECT title from books;
    SELECT insert(title, 1, 0 , 'nice ') from books;
    
    SELECT repeat('*', char_length('Stefan')) from books ;
    
    SELECT round(sum(cost),2) from books;
    
    SELECT extract(YEAR from '1996-05-05');
    
    SELECT concat_ws(' ', first_name,  last_name) as full_name, timestampdiff( day, born, died ) as days_lived from authors;
